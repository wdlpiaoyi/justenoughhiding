...
- Config files live in `config/jeh/`: `client.toml` (`JehConfig`), `listehiding.json`
  (`ListEHidingStore`), `intentoverrides.json` (`IntentOverrides`). On first run the mod creates an
  empty `listehiding.json` plus a `listehiding.example.json` (`ListEHidingStore.ensureDefaults`);
  `client.toml` is generated by Forge from `JehConfig`. The repo `defaultconfigs/jeh/` holds
  templates (`client.toml`, `listehiding.example.json`) for pack authors to copy into an instance.
