...
- EMI: `compileOnly`; discovered via `@dev.emi.emi.api.EmiEntrypoint`
  ...
  `(recipe list, captured version)`. `client/JehClientReloadEvents` clears these on every client
  resource reload and `client/JehServerReloadEvents` clears them on every server data reload
  (world join, `/reload`), both via `client/jehide/JehReloadHooks` (the manager instance is
  `final`, so identity alone is not enough). Removed packs' intents are dropped at reload time
